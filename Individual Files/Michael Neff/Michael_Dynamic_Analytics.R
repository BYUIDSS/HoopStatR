...
save_tables_to_csv <- function(connection, save_to_folder) {
  ### Saves all tables in the data base as a CSV to a specified location

  # Get all table names
  table_names <- DBI::dbListTables(connection)

  for (t in table_names) {
    # Make full path to save CSV to
    full_path <- paste0(save_to_folder, "/", t, ".csv")

    # Make sure the file doesn't exist before proceeding
    if (!file.exists(full_path)) {
      # Read in table as data frame
      table <- DBI::dbReadTable(connection, t)

      # Save table as CSV to specified folder
...
# Call My Functions
show_opposing_team_highest_shooting_position(conn, court_plot_image, "Highland Varsity 19-20")

save_tables_to_csv(conn, save_to_folder = "Hoop Stats Data")
